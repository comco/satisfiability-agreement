...
A \emph{message star-type} $\mstps\subseteq\TIamn$ over $\TI$ is a nonempty set
of $\xx$-message types such that if $\ii\in[1,\nm]$ then there are exactly
$\nMM\ii$ types $\tpt\in\mstps$ having $\smm\ii(\xx,\yy)\in\tpt$.
\end{definition}

\begin{definition}
Let $\StrA$ be a $\ClSig\SigS\sms$-structure and let $\ea\in\domA$.
The \emph{message star-type} $\mstpIa\StrA\ea$ of $\ea$ is defined by:
\[
  \mstpIa\StrA\ea = \setbd{\tpIab\StrA\ea\eb\in\TIamn}{\eb\in\domA\sub\set\ea}.
\]
Then indeed $\mstps = \mstpIa\StrA\ea$ is a message star-type over $\TI$.
\end{definition}
\begin{proof}
TODO
\end{proof}
...
\begin{definition}
Let $\nZ\geq1$.
A structure $\StrA$ for $\TI$ is \emph{$\nZ$-message star-type differentiated}
if each message star-type over $\TI$ is realized either at most once or at least
$\nZ$ times in $\StrA$.
\end{definition}