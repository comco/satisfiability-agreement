...
\begin{itemize}
  ...
  \item
  It is known that $\vFo2$ has the finite model property~{[Mortimer, 1975]} and
...
\[
  ...
  (\falpp\ii(\xx,\yy) \land \xx \neq \yy)
\]
Strategy: if $\fpsi$ is a subformula of $\fphi$ of the form $Q x
\alpha(x,y)$, where $\alpha$ is quantifier-free, then
$\fphi$ and $\fphi' \land \forall y(\spe(y)\lequ Q x \alpha(x,y))$ are
equisatisfiable, where $\spe(y)$ is a new unary symbol and $\fphi'$ is obtained
from $\fphi$ by replacing the subformula $\fpsi$ by $\spe(y)$.
\end{frame}
