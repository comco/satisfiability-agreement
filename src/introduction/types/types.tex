...
predicate signature.
For example, if $\SigS$ contains the binary predicate symbol $\se$ with intended
interpretation as an equivalence, then every $1$-type over $\SigS$ includes the
literal $\se(\xx,\xx)$.
...
If $\ea\in\domA$ and $\eb\in\domA\sub\set\ea$, the \emph{$2$-type of $(\ea,
\eb)$ in $\StrA$} is
\[
