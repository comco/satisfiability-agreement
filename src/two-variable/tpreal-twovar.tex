...
\begin{enumerate}
  ...
  there is some $\eb\in\domA\sub\set\ea$ such that $\tpIa\StrA\eb = \tpp$.
  \item
...
\begin{enumerate}
  ...
  Next suppose that $\tpk\not\in\TK\TI$, so some $\tpt\in\TI$ connects $\tpk$
  with itself. Then some $\ea\in\domA$ and $\eb\in\domA\sub\set\ea$ have
  $\tpIab\StrA\ea\eb = \tpt$, so $\tpIa\StrA\ea = \tpIa\StrA\eb = \tpk$.
  
  \item
  First let $\tpp\in\TW\TI$, so some $\tpt\in\TI$ connects $\tpp$ with itself.
  Let $\ea\in\domA$ have $\tpIa\StrA\ea = \tpp$.
  Let $\eap\in\domA$ and $\ebp\in\domA\sub\set\eap$ have
  $\tpIab\StrA\ea\eb = \tpt$.
  Then $\tpIa\StrA\ea = \tpIa\StrA\eb = \tpp$.
  Let $\eb \in \set{\eap,\ebp}\sub\set\ea$, which is nonempty.
  Then $\tpIa\StrA\eb = \tpp$ and $\eb\in\domA\sub\set\ea$.
  
  Next suppose that for every $\ea\in\domA$ having $\tpIa\StrA\ea = \tpt$, there
  is some $\eb\in\domA\sub\set\ea$ having $\tpIa\StrA\eb = \tpp$.
  Since $\tpp\in\TP\TI$, some $\ea\in\domA$ has $\tpIa\StrA\ea = \tpp$, so some
  $\eb\in\domA\sub\set\ea$ has $\tpIa\StrA\eb = \tpp$, so
  $\tpt = \tpIab\StrA\ea\eb$ connects $\tpp$ with itself, so $\tpp\in\TP\TI$.
