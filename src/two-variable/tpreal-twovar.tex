...
\begin{itemize}
  ...
  By Condition~\refcondrealizI the type $\tpIp$ is realized (possibly many
  times) in $\StrA$.
...
\end{remark}
\begin{proof}
That $\TpiPp = \TpiP$ follows from Conditions \refcondrealizI
and~\refcondrealizp for $\StrA$.
That $\TpiTp \subseteq \TpiT$ follows from Condition~\refcondrealizII for
$\StrA$.
That $\TKgi\TpiP\TpiTp = \TKgi\TpiP\TpiT$ follows from Condition~\refcondrealizk
for $\StrA$.
\end{proof}
\begin{remark}
Let $\Tpi\TpiP\TpiT$ be a type instance over $\SigS$,
...
according to~\Cref{rem:str-tpi-kings-eq}.
We verify the conditions for $\StrB$ to be a model for $\Tpi\TpiP\TpiT$:
\begin{itemize}
  \item[\refcondrealizI]
  follows from Condition~\refcondrealizI for the $\Tpi\TpiP\TpiTp$-model $\StrB$.
  \item[\refcondrealizII]
  follows from Condition~\refcondrealizII for the $\Tpi\TpiP\TpiTp$-model
  $\StrB$ together with $\TpiTp \subseteq \TpiT$, according
  to~\Cref{rem:str-tpi-kings-eq}.
  \item[\refcondrealizp]
  follows from Condition~\refcondrealizp for the $\Tpi\TpiP\TpiTp$-model
  $\StrB$.
  \item[\refcondrealizk]
  follows from Condition~\refcondrealizk for the $\Tpi\TpiP\TpiTp$-model
...
\begin{itemize}
  ...
  Note that the existence is already implied by~\refcondcertp.
...
\begin{itemize}
  \item[\refcondrealizI] follows from Condition~\refcondrealizI for $\StrA$ as a
  $\Tpi\TpiP\TpiTp$-model.
  \item[\refcondrealizII] follows from Condition~\refcondrealizII for $\StrA$ as
  a $\Tpi\TpiP\TpiTp$-model together with $\TpiTp \subseteq \TpiT$.
  \item[\refcondrealizp] follows from Condition~\refcondrealizp for $\StrA$ as a
  $\Tpi\TpiP\TpiTp$-model.
  \item[\refcondrealizk] follows from Condition~\refcondrealizk for $\StrA$ as a
  $\Tpi\TpiP\TpiTp$-model together with Condition~\refcondcertc for the
  certificate $\Cert$.
\end{itemize}
\end{proof}
...
\TpiP\TpiT[\StrA]$ according to~\Cref{rem:str-tpi-kings-eq} be the type instance
of $\StrA$.
