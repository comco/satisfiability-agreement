...
A \onetype/ $\tpw$ that is not a king type is a \emph{worker type};
...
\begin{enumerate}
  ...
  $\tpw\in\TW\TI$ iff
  some $\ea\in\domA$ and $\eb\in\domA\sub\set\ea$
  have $\tpIa\StrA\ea = \tpIa\StrA\eb = \tpw$.
  \item
...
\begin{enumerate}
  ...
  \item
  If $\tpw \in \TW\TI$ then some $\tpt\in\TI$ connects $\tpw$ with itself.
  Then some $\ea\in\domA$ and $\eb\in\domA\sub\set\ea$
  have $\tpIab\StrA\ea\eb = \tpt$, so $\tpIa\StrA\ea = \tpIa\StrA\eb = \tpw$.
  
  If some $\ea\in\domA$ and $\eb \in \domA\sub\set\ea$
  have $\tpIa\StrA\ea = \tpIa\StrA\eb = \tpw$,
  then $\tpt = \tpIab\StrA\ea\eb \in \TI$ connects $\tpw$ with itself.
  ...
  so some $\eap \in \domA$ and $\ebp \in \domA\sub\set\eap$ have
  $\tpIa\StrA\eap = \tpIa\StrA\ebp = \tpp$.
  Let $\eb \in \set{\eap,\ebp}\sub\set\ea$, which is nonempty.
  Then $\tpIa\StrA\eb = \tppp$ and $\eb\in\domA\sub\set\ea$.
\end{enumerate}
\end{proof}
