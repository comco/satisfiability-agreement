% Cosmic spectrum
\DeclareMathOperator{\cspOP}{csp}
\newcommand{\cspIX}[2]{\cspOP^{#1}[#2]}

% galactic types
\newcommand{\TpTg}[1]{\Tau_{\mathrm{g}}[#1]}
% cosmic types
\newcommand{\TpTc}[1]{\Tau_{\mathrm{c}}[#1]}

% galactically connectable
\newcommand{\gconn}[1]{\sim_{\mathrm{g}}^{#1}}
% cosmic connectable
\newcommand{\cconn}[1]{\sim_{\mathrm{c}}^{#1}}

\DeclareMathOperator{\TpOP}{Tp}

% x-type
\newcommand{\xTp}[1]{\TpOP_{\xx}#1}
\newcommand{\yTp}[1]{\TpOP_{\yy}#1}

\newcommand{\si}{\predname{in}}

% black hole
\newcommand{\sbh}{\predname{bh}}

\newcommand{\noe}[1]{#1^{-}}

\newcommand{\tin}[1]{{#1}^{\mathcal{I}}}
\newcommand{\tout}[1]{{#1}^{\mathcal{O}}}

% TODO: remove spe
\newcommand{\spe}{\predname{p}}
\newcommand{\spp}[1]{\predname{p}^{#1}}

% sets
\newcommand{\sPe}[1]{\predname{p}_{#1}}
% married types
\newcommand{\mr}[2]{{#1}_{#2}}
\newcommand{\mrr}[3]{{#1}_{#2#3}}

% cosmic spectrum
\newcommand{\csps}{\sigma}
\newcommand{\cspsp}{{\csps'}}

\newcommand{\condcspx}{$(\csps\xx)$}
\newcommand{\refcondcspx}{\hyperref[cond:cspx]{\condcspx}}

\newcommand{\condcspnx}{$(\csps\tpIn\xx)$}
\newcommand{\refcondcspnx}{\hyperref[cond:cspnx]{\condcspnx}}

\newcommand{\condcspny}{$(\csps\tpIn\yy)$}
\newcommand{\refcondcspny}{\hyperref[cond:cspny]{\condcspny}}

\newcommand{\condcspm}{$\bcancel{(\csps\sm)}$}


\newcommand{\certcondIp}{$(\Cert1')$}
\newcommand{\certcondIIp}{$(\Cert2')$}
\newcommand{\certcondIIIp}{$(\Cert3')$}
\newcommand{\certcondIVp}{$(\Cert4')$}

\newcommand{\refcertcondIp}{\hyperref[cond:cert-Ip]{\certcondIp}}
\newcommand{\refcertcondIIp}{\hyperref[cond:cert-IIp]{\certcondIIp}}
\newcommand{\refcertcondIIIp}{\hyperref[cond:cert-IIIp]{\certcondIIIp}}
\newcommand{\refcertcondIVp}{\hyperref[cond:cert-IVp]{\certcondIVp}}

\newcommand{\TpiPs}[1]{\TpiP^{#1}}
\newcommand{\TpiTs}[1]{\TpiT^{#1}}
\newcommand{\TpiTc}{\TpiT_{\mathrm{c}}}
%\newcommand{\TpiTpc}{\TpiT_{\mathrm{c}}'}
\newcommand{\TpiTg}{\TpiT_{\mathrm{g}}}

\newcommand{\tIN}{\mathcal{I}}
\newcommand{\tOUT}{\mathcal{O}}
\newcommand{\tBH}{\mathcal{B}}

\newcommand{\sticondI}{$(\tIN)$}
\newcommand{\sticondO}{$(\tOUT)$}
\newcommand{\sticondB}{$(\tBH)$}
\newcommand{\sticondII}{$(\tIN\tIN)$}
\newcommand{\sticondIO}{$(\tIN\tOUT)$}
\newcommand{\sticondOO}{$(\tOUT\tOUT)$}
\newcommand{\sticondIB}{$(\tIN\tBH)$}
\newcommand{\sticondOB}{$(\tOUT\tBH)$}

\newcommand{\refsticondI}{\hyperref[sti-I]{\sticondI}}
\newcommand{\refsticondO}{\hyperref[sti-O]{\sticondO}}
\newcommand{\refsticondB}{\hyperref[sti-B]{\sticondB}}
\newcommand{\refsticondII}{\hyperref[sti-II]{\sticondII}}
\newcommand{\refsticondIO}{\hyperref[sti-IO]{\sticondIO}}
\newcommand{\refsticondOO}{\hyperref[sti-OO]{\sticondOO}}
\newcommand{\refsticondIB}{\hyperref[sti-IB]{\sticondIB}}
\newcommand{\refsticondOB}{\hyperref[sti-OB]{\sticondOB}}

\newcommand{\StrAs}[1]{\StrA^{#1}}

% Sets of galaxies
\newcommand{\Gals}{\mathcal{G}}
\newcommand{\GalsN}{\mathcal{G}_\TNo}
\newcommand{\GalsP}{\mathcal{G}_\TPs}
\newcommand{\Galsp}{{\Gals'}}
\newcommand{\GalsNp}{{\GalsN'}}
\newcommand{\GalsPp}{{\GalsP'}}
% Galaxies
\newcommand{\galX}{X}
\newcommand{\galXX}[1]{\galX_{#1}}
\newcommand{\galXp}{X'}
\newcommand{\galY}{Y}
\newcommand{\galYp}{Y'}
